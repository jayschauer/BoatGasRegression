...
ridge_regressor = ElasticNetCV(fit_intercept=True, positive=True,
                               ...
                               l1_ratio=0, alphas=np.arange(0.0001, 0.01, 0.0001))
elastic_regressor = ElasticNetCV(fit_intercept=True, positive=True,
                                 l1_ratio=np.arange(0.001, 1.0, 0.001),
                                 tol=1e-5, max_iter=10000)
...
ridge_model = ridge_regressor.fit(X, y)
elastic_model = elastic_regressor.fit(X, y)
...
print(f'R^2: {linear_model.score(X, y)}')
print(f'prediction: {linear_model.predict(to_predict)}')
print()
print('Ridge model:')
print(f'coefficients: {ridge_model.coef_}')
print(f'intercept: {ridge_model.intercept_}')
print(f'l1 ratio: {ridge_model.l1_ratio_}')
print(f'alpha: {ridge_model.alpha_}')
print(f'R^2: {ridge_model.score(X, y)}')
...
print(f'R^2: {elastic_model.score(X, y)}')
